...
def get_ssl_results(train_loss, test_loss, test_acc,result_dir, model, num_classes, step, filep, num_epochs, args, labels_per_class, img_shape):
        ...
        #c_optimizer = torch.optim.Adadelta(C.parameters(), lr=0.5, rho=0.9, eps=1e-06, weight_decay=0.0001)
        #c_optimizer = torch.optim.RMSprop(C.parameters(), lr=0.01, alpha=0.99, eps=1e-08, weight_decay=0, momentum=0, centered=False)
