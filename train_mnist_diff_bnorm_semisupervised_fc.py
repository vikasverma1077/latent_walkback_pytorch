...
def parse_args():
    ...
    parser.add_argument('--alpha4', type=float, default=1.0,help='coefficient for prior (p(y))')
...
class Net(nn.Module):
    def __init__(self, args,input_shape=(1,hw_size,hw_size)):
        ...
        self.fc_x_z_1 = nn.Linear(784+num_classes, 600)
        self.encoder_params.extend(self.fc_x_z_1.parameters())
        self.bn1_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn1_list.append(nn.BatchNorm1d(600))
            self.encoder_params.extend(self.bn1_list[i].parameters())
        
        self.fc_x_z_2 = nn.Linear(600, 600)
        self.encoder_params.extend(self.fc_x_z_2.parameters())
        self.bn2_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn2_list.append(nn.BatchNorm2d(600))
            self.encoder_params.extend(self.bn2_list[i].parameters())
        
           
        self.fc_z_mu = nn.Linear(600, args.nl)
        self.encoder_params.extend(self.fc_z_mu.parameters())
        self.bn3_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn3_list.append(nn.BatchNorm1d(args.nl))
            self.encoder_params.extend(self.bn3_list[i].parameters())
        
        self.fc_z_sigma = nn.Linear(600, args.nl)
        self.encoder_params.extend(self.fc_z_sigma.parameters())
        self.bn4_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn4_list.append(nn.BatchNorm1d(args.nl))
        ...
        for i in xrange(args.meta_steps):
            ...
            self.transition_params.extend(self.bn5_list[i].parameters())
            #print _
        ...
        for i in xrange(args.meta_steps):
            ...
            self.transition_params.extend(self.bn6_list[i].parameters())
        
        self.fc_trans_1_2 = nn.Linear(1024, 600)
        self.transition_params.extend(self.fc_trans_1_2.parameters())
        self.bn7_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn7_list.append(nn.BatchNorm1d(600))
        ...
        self.fc_z_x_1 = nn.Linear(args.nl+num_classes, 600)
        self.decoder_params.extend(self.fc_z_x_1.parameters())
        self.bn8_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn8_list.append(nn.BatchNorm1d(600))
            self.decoder_params.extend(self.bn8_list[i].parameters())
            
            
        self.fc_z_x_2 = nn.Linear(600, 600)
        self.decoder_params.extend(self.fc_z_x_2.parameters())
        self.bn9_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn9_list.append(nn.BatchNorm1d(600))
            self.decoder_params.extend(self.bn9_list[i].parameters())
            
            
        self.fc_z_x_3 = nn.Linear(600, 784)
        self.decoder_params.extend(self.fc_z_x_3.parameters())
        self.bn10_list=nn.ModuleList()
        for i in xrange(args.meta_steps):
            self.bn10_list.append(nn.BatchNorm2d(784))
            self.decoder_params.extend(self.bn10_list[i].parameters())
            
            
        
    def encode(self, x,y, step):
        
        h1 = self.act(self.bn1_list[step](self.fc_x_z_1(x)))
        #print c1
        h2 = self.act(self.bn2_list[step](self.fc_x_z_2(h1)))
        #print h2.shape
        mu = self.bn3_list[step](self.fc_z_mu(h2))
        sigma = self.bn4_list[step](self.fc_z_sigma(h2))
        return mu, sigma
    ...
    def transition (self, z, temperature, step):
        ...
        h1 = self.act(self.bn5_list[step](self.fc_trans_1(z)))
        #print h1
        h2 = self.act(self.bn6_list[step](self.fc_trans_1_1(h1)))
        #print h2
        h3 = self.act(self.bn7_list[step](self.fc_trans_1_2(h2)))
        
        #print h3
        h3 = torch.clamp(h3, min=0, max=5)
        #print h3
        #print h3.shape
        mu = self.bn3_list[step](self.fc_z_mu(h3))  #### why not non-linearity applied here
        #print mu
        sigma = self.bn4_list[step](self.fc_z_sigma(h3))
        #print sigma
    ...
    def decode (self, z_new, step):
        #print z_new
        d0 = self.act(self.bn8_list[step](self.fc_z_x_1(z_new)))
        #print d0
        #d0 = d0.view(-1, 256, 3, 3)
        d1 = self.act(self.bn9_list[step](self.fc_z_x_2(d0)))
        #print d1.data.shape
        d2 = self.sigmoid(self.bn10_list[step](self.fc_z_x_3(d1)))
        #print d2.data.shape
...
def compute_loss(x, y, z, y_dim, model, loss_fn, start_temperature, meta_step, encode= False):
    
    is_labelled = False if y is None else True
    # Enumerate choices of label
    if not is_labelled:
        y = enumerate_discrete(x, y_dim)
        x = x.repeat(y_dim, 1)
    ...
    z_tilde = torch.cat([z_tilde, y], dim=1)
    ...
    prior = log_standard_categorical(y)### p(y)
    
    loss =  -log_p_reverse*args.alpha2 + x_loss*args.alpha1 + prior*args.alpha4
...
def train(args,
    ...
    for epoch in range(args.epochs):
        ...
        for (data, target), (u, _) in zip(cycle(train_loader), unlabelled_loader):
            batch_idx +=1 
            if args.cuda:
                data, target, u = data.cuda(), target.cuda(), u.cuda()
            data, target, unlabelled_data = Variable(data), Variable(target), Variable(u)
            ...
            if batch_idx%5==0:
                ...
                for num_step in range(args.num_steps * args.meta_steps):
                    ...
                    data = data_forward_diffusion.view(-1,1, spatial_width, spatial_width)#reshape(args.batch_size, n_colors, WIDTH, WIDTH)
                    if num_step%2==1:
                        plot_images(data.data.cpu().numpy(), model_dir + '/' + "batch_" + str(batch_idx) + '_corrupted_' + 'epoch_' + str(epoch) + '_time_step_' + str(num_step))
