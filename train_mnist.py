...
hw_size=28 ## height and width of MNIST data
...
class Net(nn.Module):
    def __init__(self, args,input_shape=(1,hw_size,hw_size)):
        ...
        self.conv_x_z_1 = nn.Conv2d(1, 16, kernel_size=kernel_size, stride=2, padding=padsize)
        self.bn1 = nn.BatchNorm2d(16) 
        self.conv_x_z_2 = nn.Conv2d(16, 32, kernel_size=kernel_size, stride=2, padding=padsize)
        ...
        self.fc_layer_1 = nn.Linear(32*7*7, 256)
        self.bn4 = nn.BatchNorm1d(256)
        
        self.fc_z_mu = nn.Linear(256, args.nl)
        self.fc_z_sigma = nn.Linear(256, args.nl)
        
        ###### transition operator ########
        self.fc_trans_1 = nn.Linear(args.nl, 256)
        self.bn5 = nn.BatchNorm1d(256)
        self.fc_trans_1_1 = nn.Linear(256, 256)
        ...
        ### decoder #####
        self.fc_z_x_1 = nn.Linear(args.nl, 32*7*7)
        self.bn8 = nn.BatchNorm1d(32*7*7)
        self.conv_z_x_2 = nn.ConvTranspose2d(32, 16, kernel_size=4, stride= 2, padding=1)### change to deconv
        self.bn9 = nn.BatchNorm2d(16)
        self.conv_z_x_3 = nn.ConvTranspose2d(16, 1, kernel_size=4, stride= 2, padding=1)### change to deconv
        self.bn10 = nn.BatchNorm2d(1)
        #self.conv_z_x_4 = nn.conv2d(64, 3, kernel_size=5, stride= -2)### change to deconv
    ...
    def encode(self, x):
        ...
        #print (c2.data.shape)
        h1 = c2.view(-1,32*7*7)# self.flat_shape)
    ...
    def decode (self, z_new):
        ...
        #print (d2.data.shape)
        p =  d2.view(-1, 1*28*28)## TODO: Change the flat shape : 1*32*32???
    ...
    def sample(self, z, temperature):
        ...
        #d3 = self.sigmoid(self.conv_z_x_4(d2))
        x_new =  d2.view(-1, self.get_flat_shape(input_shape=(258,8,8)))## TODO: Change the flat shape : 1*32*32???
...
def compute_loss(x, model, loss_fn, start_temperature):
    ...
    #print (mu.data.shape, logvar.data.shape)
    z = model.reparameterize(mu, logvar)
    z_tilde, log_p_reverse, sigma, h2 = model.transition( z, temperature)
    #print z_tilde.data.shape
    x_tilde = model.decode(z_tilde)
    #print (x.data.shape)
    x_loss = loss_fn(x_tilde, x.view(-1, 1*28*28))## sum over axis=1
    loss = log_p_reverse + x_loss
    x_states = [x_tilde]
    z_states = [z_tilde]
    
    #print (x_tilde.data.shape, z_tilde.data.shape)
    total_loss = loss
    ...
    for _ in range(args.num_steps - 1):
        ...
        mu, logvar = model.encode(x_tilde.view(-1, 1, 28, 28))
        z = model.reparameterize(mu, logvar)
        z_tilde, log_p_reverse, sigma, h2 = model.transition( z, temperature)
        x_tilde = model.decode(z_tilde)
        #print (x.data.shape, x_tilde.data.shape)
        x_loss = loss_fn(x_tilde,x)## sum over axis=1
        #print x_loss.data.shape, log_p_reverse.data.shape
        loss = log_p_reverse + x_loss
...
def train(args,
    ...
    """
    dataset='mnist'
    data_source_dir = home+'data/DARC/'+dataset+'/'
    if not os.path.exists(data_source_dir):
        os.makedirs(data_source_dir)
    data_target_dir = tmp+'data/DARC/'+dataset+'/'
    ...
    if args.dataset == 'MNIST':
        print 'loading MNIST'
        train_loader, test_loader=load_mnist(args.data_aug, args.batch_size, args.batch_size,args.cuda, data_target_dir)
        n_colors = 1
        spatial_width = 28
