...
def get_batch_samples(fixed_latent_factor_index, batch_size):
...
def train_classifier(num_batches, num_z):
    ...
    for i in range(num_batches):
        index = select_fixed_latent_factor_index(latents_sizes)
        y[i]= index-2 ## substracted 2 so that the class index starts from zero
        #print (index)
        x[i,:] = get_batch_samples(index, batch_size=10)##  difference defined at point 3 page 7 https://openreview.net/pdf?id=Sy2fzU9gl
    #print (y)
    #print (x)  
    ...
    C.cuda()
    loss_fn = nn.NLLLoss()
    optimizer = optim.SGD(C.parameters(), lr=0.001, momentum=0.9, weight_decay=0)
