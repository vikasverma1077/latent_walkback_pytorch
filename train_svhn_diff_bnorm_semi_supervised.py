##modified from train_svhn_diff_bnorm
import argparse
import numpy as np
import os
import mimir
...
def train(args,
    ...
    train_loader, unlabelled_loader, test_loader, extra_loader, num_classes = load_data(args.data_aug, batch_size= args.batch_size ,workers=2, dataset=dataset, data_target_dir= '/u/vermavik/data/DARC/svhn')
    n_colors = 3
    spatial_width = 32
    ...
    for epoch in range(args.epochs):
        print ('epoch', epoch)
        #for batch_idx, (data, target) in enumerate(train_loader):
        for (data, target), (u, _) in zip(cycle(labelled), unlabelled):
            if args.cuda:
                data, target, u = data.cuda(), target.cuda(), u.cuda()
            data, target = Variable(data), Variable(target), Variable(u)
