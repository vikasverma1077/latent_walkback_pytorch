...
def get_ssl_results(model, num_classes, train_loader, test_loader, step, filep, num_epochs):
        C = nn.Sequential(
...
def train(args,
    ...
    train_loader, unlabelled_loader, test_loader, extra_loader, num_classes = load_data(args.data_aug,labels_per_class=400, batch_size= args.batch_size ,workers=2, dataset=dataset, data_target_dir= '/u/vermavik/data/DARC/svhn')
    n_colors = 3
    spatial_width = 32
    ...
    for epoch in range(args.epochs):
        print ('epoch', epoch)
        #for batch_idx, (data, target) in enumerate(train_loader):
        for batch_idx, (data, target) in enumerate (unlabelled_loader):
            #print data
            if args.cuda:
                data = data.cuda()
            data = Variable(data)
            ...
            if batch_idx%500==0:
                ...
                get_ssl_results(model, num_classes, train_loader, test_loader, step=0, filep = filep, num_epochs=20)
